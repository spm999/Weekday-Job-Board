...
const AllFilters = ({ allJobs, setFilteredJobs }) => {
    ...
    const applyFilters = () => {
        ...
        // Set noJobsMessage only if filters are applied and no matching jobs
        setNoJobsMessage(
            Object.values(filters).some(filter => filter !== '') && filteredJobs.length === 0
        );
    };
    ...
    return (
        <div>
            ...
            {/* {noJobsMessage && <p style={{ textAlign: 'center', padding: '20px', backgroundColor: 'red' }} className="no-jobs-message">No jobs match the applied filters.</p>} */}
            {noJobsMessage && <p className="no-jobs-message">No jobs match the applied filters.</p>}
